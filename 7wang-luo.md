...
管理员可创建外部网络，一个外部网络允许添加多个子网，申请浮动IP时，用户可以选择子网，或者指定特定IP。

一个网络区域允许创建一个外部网络。路由绑定外部网络时，选择绑定在某个网络上，也可以选择某一个子网，路由外部网关只绑定在一个外部端口。
...
点击左侧导航栏“网络->外部网络管理”，进入外部网络管理页面，即可查看当前外部网络列表，列表所含字段具体如下：

<<<<<<< HEAD

* 名称：外部网络的名字，由用户创建网络时填写。
* 状态：该字段显示当前网络是否正常可用。
* 网络分区：外部网络所属的网络区域。
* 物理网络：网络所关联的
物理网络。
...
在外部网络列表中，用户可通过输入网络名称进行精确查询，也可按照状态和网络分区进行筛选。

### 7.1.3 外部网络相关操作 {#7-1-3}

针对外部网络的操作包括：创建外部网络、编辑外部网络、删除外部网络。
...
>>>>>>> origin/changes/1
| :--- | | :--- | | :--- |
| 创建外部网络 | 平台已部署至少一个网络区域 | 点击右上角“创建外部网络”按钮，在弹窗中输入网络名称，选择网络区域和物理网络，填写子网名称、网络地址、网关IP，设置一个或多个IP段，最后点击“确定”按钮。 |
| 编辑外部网络 | -- | 在要编辑的外部网络右侧，点击编辑图标，可在弹出框中更改网络名称。 |
| 删除外部网络 | 网络下没有子网 | 点击要删除外部网络右侧的删除图标，在弹窗中点击“删除”按钮即可。| 

### 7.1.4 外部网络详情 {#7-1-4}

点击外部网络的名称可进入外部网络的详情页面，查看更多信息。

外部网络详情页包含两部分：基本信息及子网列表。

* 基本信息：包括外部网络名称、UUID、状态、网络区域、浮动IP、路由数量。
* 子网列表：包括子网名称、子网网段、网关IP、IP版本。

### 7.1.5 子网相关操作 {#7-1-5}

针对外部网络子网的操作包括：创建子网、编辑子网、删除子网。
...
>>>>>>> origin/changes/1
| :--- || :--- |
| 创建子网 | 点击子网列表右上角“创建外部网络”按钮，在弹窗中填写子网名称、网络地址、网关IP，设置一个或多个IP段，最后点击“确定”按钮。 |
| 编辑子网 | 在要编辑的子网右侧，点击编辑图标，可在弹出框中更改子网名称，添加IP段。 |
| 删除子网 | 点击要删除子网右侧的删除图标，在弹窗中点击“删除”按钮即可。| 
...
>>>>>>> origin/changes/1
| :--- | | :--- | | :--- |
| 创建外部网络 | 平台已部署至少一个网络区域 | 点击右上角“创建网络”按钮，在弹窗中输入网络名称，选择网络区域，填写子网名称、网络地址、网关IP，然后选择一个匹配的路由，最后点击“确定”按钮。 |
| 编辑外部网络 | -- | 选中一个网络，点击右上角“更多操作->编辑网络”，可在弹出框中更改网络名称。 |
| 删除外部网络 | 网络下没有子网 | 选中一个网络，若它包含的子网数为零，点击右上角“更多操作->删除网络”。 |
...
针对浮动IP的操作包括：创建浮动IP、编辑浮动IP、删除浮动IP、关联端口以及取消关联端口。
...
|
| 关联管理 | -- | 选中一个虚拟IP，点击右侧“关联管理”，在弹出框中可增删要关联的主机。</br>注意：每个虚拟IP最多可关联5个主机网卡，每个网卡可被多个虚拟IP关联（没有数量限制）。
|
| 释放虚拟IP |虚拟IP关联的主机网卡数为0 | 在虚拟IP列表中，点击要删除的浮动IP右侧的“释放虚拟IP”，弹窗提示是否确定释放该虚拟IP，点击“确定”完成释放。|
...
| 删除防火墙 | 选中防火墙，在更多操作下拉框中选择“删除”，弹窗提示是否确定删除该防火墙，点击“确定”完成。
...
| 操作 |描述 |
| :--- || :--- |
| 创建安全组 | 点击安全组列表右上角的“创建安全组”按钮，在弹窗中填写安全组名称和描述信息，点击“确定”完成创建。|
...
| 操作 |描述 |
| :--- || :--- |
| 创建负载均衡 | 点击列表右上角的“创建负载均衡”按钮，在弹窗中填写负载均衡器的名称和描述信息，点击“确定”完成创建。|
| 编辑负载均衡 | 选中要编辑的负载均衡，点击右上角的“更多操作”按钮，在下拉框中选择“编辑”，在弹窗中修改名称和描述信息，点击“确定”按钮完成修改。|
| 删除负载均衡 | 选中要删除的负载均衡，点击右上角的“更多操作”按钮，在下拉框中选择“删除”，在确认框中点击“删除”按钮完成删除。| 

### 7.9.3 负载均衡详情 {#7-9-3}

点击负载均衡名称，可查看该负载均衡器的详细描述，包括“基本信息”和“监听器”。

* 基本信息包括：名称、描述、标识、项目标识、状态、子网、网络分区和订单信息，点击右上角的操作按钮，可修改负载均衡器的名称和描述。
...
当前KingStack平台默认支持IPSec VPN，用于租户将云资源与外部IT资源进行打通和互访。其中此VPN服务可与传统的硬件防火墙、公有云VPN以及IPSec VPN软客户端进行互联。
...
登录控制台，点击左侧导航栏“网络->VPN管理”，进入到虚拟专用网络页面。


对VPN管理的操作包括：IKE策略的创建/删除、IPSec策略的创建/删除、VPN服务的创建/删除/更新、IPSec站点连接的创建/删除/更新。


| 操作 |描述 |
| :--- || :--- |
| 创建IKE策略 | 点击“IKE策略->创建IKE策略”按钮，在弹出框中输入名称、描述，按照创建向导选择授权算法、加密算法、IKE版本、生存期单位、完美前传保密、协商模式，填写生存期值（60-3600），最后点击“确定”按钮即可。|
| 创建IPSec策略 | 按照创建向导操作，基本同上。|
| 创建VPN服务 | 点击“VPN服务->创建VPN服务”按钮，在弹出框中输入名称、描述、选择路由及子网，开启“监管状态”，最后点击“确定”按钮即可。|
| 创建IPSec站点链接 | 击“IPSec站点链接->创建IPSec站点链接”按钮，在弹出框中输入名称、描述、选择刚刚创建的ike策略、ipsec策略、vpn服务；按照向导输入伙伴网关、路由器身份、对等子网、PSK字符串，然后点击下一步，按照创建向导设定，最后点击“确定”按钮，完成IPSec站点链接的创建。|

![](/assets/添加IKE策略.png)

![](/assets/添加IPSec策略.png)

![](/assets/添加VPN服务.png)

![](/assets/添加IPSec站点连接-第一步.png)

![](/assets/添加IPSec站点连接-第二步.png)

### 7.10.2 注意事项

1. 删除IKE策略/IPSec策略/VPN服务的前提是没有绑定的IPSec站点链接。
2. 点击已经创建IKE策略/IPSec策略/VPN服务名称，可在弹窗层中看到关于它的详细信息，包括：描述、UUID、项目ID。
