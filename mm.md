...
3. 配置网络。点击具体节点右侧的“配置网络”功能链接，可在弹出框中执行发现网卡、编辑网卡信息和配置bond的操作。当已为节点指定角色并完成部署后，“配置网络”功能将禁用不可点击。
4. 指定节点角色。在节点列表中选定指定的节点，点击“部署节点”。在弹出框中勾选节点类型，同时填写节点的网卡信息以及存储网络信息。
...
| 重启 | 重新启动物理节点 |
| 重装系统 | 重装计算节点系统，该操作只可在部署节点之前执行 |
| 卸载网络节点 | 卸载节点上已经部署好的网络相关组件 | 
| 卸载计算节点 | 卸载节点上已经部署好的计算虚拟化相关组件 |
