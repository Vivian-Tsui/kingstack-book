...
虚拟网络结构主要展示一些基础虚拟网络资源配置，并开放一部分自定义配置。通过虚拟网络管理页面，admin可查看虚拟网络配置并执行相关操作。

### 9.8.1 虚拟网络列表 {#9-8-1}

点击左侧菜单栏中的“管理”，选择“虚拟网络管理”，进入虚拟网络管理页面。目前暂时只支持vlan网络类型。

vlan网络列表展示了现有ID段、已被占用的ID，以及与其对应的物理网络和关联网络。

### 9.8.2 添加ID段 {#9-8-2}

点击列表右上角的“添加ID段”按钮，在弹出框中输入ID段的起始值和结束值，点击“确定”创建ID段。

如果新建ID段与某个已有ID段是连续的，则两个ID段合为一个ID段显示在列表中。

### 9.8.3 删除ID段 {#9-8-3}

点击列表右上角的“删除ID段”按钮，在弹出框中输入要删除ID段的起始值和结束值，点击“删除”按钮删除所选ID段。

如果所输入的ID段中有部分ID绑定了网络，绑定网络的ID不会被删除。

如果列表中的某个ID段下并未绑定网络，则可直接点击该ID段右侧的删除图标，快速删除该ID段。
...
监控服务器管理旨在实现zabbix分布式监控部署，进而提升数据库性能和可管理性。

### 9.10.1 根监控服务器 {#9-10-1}

1. 以admin或者maintenanceadmin账户登录控制台，建议采用后者。
2. 点击左侧菜单栏的“管理->监控服务器管理”，进入监控服务器管理页面，页面默认展示“根监控服务器”选项卡的内容。 

根监控服务器列表以整体集群的方式展示，点击集群名称可以进入详情页，点击下拉列表按钮则会展开每台根监控服务器的信息，包括名称、状态、IP、创建时间。

#### 9.10.1.1 探测根监控服务器 {#9-10-1-1}

点击列表右上方的“探测根监控服务器”按钮，平台将探测现有根监控服务器，并将新探测到的监控服务器信息展现在列表中。

#### 9.10.1.2 根监控服务器详情 {#9-10-1-2}

点击集群名称可进入根监控服务器详情页，在此可查看当前所监控的计算节点、物理机、控制节点，以及交换机等。 

在详情页，可针对计算节点、物理机和控制节点执行监控项迁移操作，其中可批量迁移计算节点的监控项。

![](/assets/迁移监控项-计算节点.png)

迁移物理机监控项时，需输入物理机的用户名和密码：

![](/assets/迁移监控项-物理机.png)

### 9.10.2 子监控服务器 {#9-10-2}

1. 以admin或者maintenanceadmin账户登录控制台，建议采用后者。
2. 点击左侧菜单栏的“管理->监控服务器管理”，进入监控服务器管理页面，点击“子监控服务器”选项卡，查看子监控服务器列表。 

子监控服务器列表包含子监控服务器的名称、状态、IP、创建时间以及相关操作。

#### 9.10.2.1 创建子监控服务器 {#9-10-2-1}

点击列表右上方的“创建子监控服务器”按钮，在弹窗中输入监控服务器名称、选择关联物理机、填写用户名及密码，之后选择监控模板和通知组（通知组为可选字段），点击“确定”按钮创建监控服务器。

#### 9.10.2.2 子监控服务器详情 {#9-10-2-2}

点击子监控服务器名称可进入详情页，在此可查看zabbix sever的基本信息，包括名称、UUID、IP、所属物理机、状态和创建时间，以及已在该服务器上创建监控的计算节点、物理机、控制节点等。 
...
#### 9.10.2.3 其他操作 {#9-10-2-3}

| 操作 | 对应按钮 | 前提条件 | 描述 |
| :--- | | :--- | | :--- | | :--- |
| 开启 | 运行 | 监控服务器的状态为“已关闭” | 开启子监控服务器 |
| 关闭 | 停止 | 监控服务器的状态为“运行中” | 关闭子监控服务器 |
| 编辑 | ![](/assets/icon-edit.png) | -- | 修改子监控服务器的名称 |
| 删除 | ![](/assets/icon-delete.png) | -- | 删除子监控服务器，删除监控服务器后，历史监控数据将被删除 |
| 更新密码 | ![](/assets/icon-more.png) | -- | 更新物理机的密码信息 |
...
虚拟机的自动恢复功能旨在提高整个数据和服务的高可用性。借助自动evacuate功能，可在物理机宕机或系统故障发生后，实现虚拟机恢复的自动性，有效的保证上层业务的可用性。

### 9.14.1 自动恢复服务基本信息 {#9-14-1}

以admin或者maintenanceadmin账户登录控制台，建议采用后者。点击左侧菜单栏的“管理->自动恢复管理”，进入自动恢复管理页面，默认选中“基本信息”选项卡。

该选项卡下展示了自动恢复服务的基本配置信息和判定规则：

* 基本信息包括服务的UUID、创建时间、服务状态、服务类型、执行时间和触发阈值。
* 判定规则是系统用来确认故障的准则，通过从不同途径获取的结果来避免误判的产生。判定规则列表列出了规则UUID、执行方式、执行结果和创建时间。
...
| :--- | | :--- |
| 启用/禁用 | 启用/禁用自动恢复服务。 |
| 修改触发阈值 | 点击触发阈值字段后方的编辑图标可修改触发阈值。 |

### 9.14.2 故障事件 {#9-14-2}

以admin或者maintenanceadmin账户登录控制台，建议采用后者。点击左侧菜单栏的“管理->自动恢复管理”，进入自动恢复管理页面，选择“故障事件”选项卡。

故障事件列表列出了已检测到的故障事件，管理员可通过输入主机名精确搜索相关故障事件，此外也可按照故障事件的状态进行筛选。

故障事件列表涵盖的信息包括故障事件的UUID、相关物理机、状态、故障时间，以及更新时间。

#### 故障事件相关操作

| 操作 | 描述 |
| :--- | | :--- |
| 添加故障事件 | 管理员可手动添加可能出现故障的主机。 |
| 删除故障事件 | 可删除单个故障事件，也可批量删除。 |
